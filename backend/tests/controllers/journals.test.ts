...
beforeAll(async () => {
  if (!process.env.MONGO_CONNECTION_STRING_TEST) {
    throw new Error(
      "MONGO_CONNECTION_STRING_TEST environment variable is not set."
    );
  }
...
describe("Journal API Endpoints", () => {
  ...
  afterEach(async () => {
    ...
    await Promise.all(
      journalIds.map((id) =>
        request(app).delete(`/api/journals/${id}`).set("Cookie", sessionCookie)
      )
    );
    journalIds = [];
  });
  ...
  it("should create a journal", async () => {
    ...
    expect(newJournalResponse.body.journalEntry).toEqual(
  ...
  it("should get all journals", async () => {
    ...
    const retrievedJournalIds = getAllResponse.body.map(
      (journal: { _id: string }) => journal._id
    );
  ...
  it("should update a journal entry", async () => {
    ...
    expect(updatedJournalResponse.body.journalEntry).toEqual(
      "hi i am updating this entry"
    );
    expect(formatDate(new Date(updatedJournalResponse.body.date))).toEqual(
      today
    );
    ...
    expect(fetchUpdatedJournalResponse.body.journalEntry).toEqual(
      "hi i am updating this entry"
    );
    expect(formatDate(new Date(updatedJournalResponse.body.date))).toEqual(
      today
...
describe("User not authenticated errors", () => {
  ...
  it("should get user not authenticated for getting journal by year", async () => {
    await request(app).get(`/api/journals/2023`).expect(401);
  });

  it("should get user not authenticated for getting journal by year and month", async () => {
    await request(app).get(`/api/journals/2023/10`).expect(401);
  });
